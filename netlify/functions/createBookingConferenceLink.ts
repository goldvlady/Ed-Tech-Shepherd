...
const createBookingConferenceLink = async () => {
    ...
    await Promise.all(bookingsWithoutLink.map(async (booking) => {
        const { roomUrl, hostRoomUrl } = await wherebyService.createMeeting();
        await Booking.updateOne({
            _id: booking._id
        }, { conferenceHostRoomUrl: hostRoomUrl, conferenceRoomUrl: roomUrl, status: STATUS.CONFIRMED });
    }))
...
export const handler = schedule("* * * * *", middy(createBookingConferenceLink) as unknown as Handler);