...
const Scheduler: React.FC<Props> = ({ value, onChange, schedule }) => {
    ...
    const [state, setState] = useState<{ scroller: any, itemWidth: number, isPrevHidden: boolean, isNextHidden: boolean }>({
        scroller: null,
        itemWidth: 0,
        isPrevHidden: false,
        isNextHidden: false
    ...
    const next = () => {
        state.scroller?.scrollBy({ left: (state.itemWidth) * 1, top: 0, behavior: 'smooth' });
    }
    ...
    return <Root>
        <DateRow ref={carouselRef}>
            {!state.isPrevHidden && <SliderNavBtn onClick={e => prev()} className="left"><FiChevronLeft /></SliderNavBtn>}
            {!state.isNextHidden && <SliderNavBtn onClick={e => next()} className="right"><FiChevronRight /></SliderNavBtn>}
            <DateRowDates className="scrollbar-hidden" ref={containerRef}>
                {schedule.map(d => {
                    const formattedDate = d.date.format('MMM D');

                    return <DateCol key={formattedDate}>
                        <DateColHeader>
                            <Text fontWeight={500} variant={"muted"} textTransform={"uppercase"} fontSize={"small"}>{d.date.format('ddd')}</Text>
                            <Text fontSize={"md"}>{formattedDate}</Text>
                        </DateColHeader>
                        <DateColContent>
                            {!isEmpty(d.slots) ? d.slots.map(slot => {
                                const active = !!find(value, slot);
                                return <SlotButton key={slot.begin} $active={active} onClick={() => toggleArrayValue(slot)}>
                                    <Checkbox pointerEvents={"none"} size="lg" isChecked={active} variant={"looney"} />
                                    <Box display={"flex"} alignItems="center">
                                        <Text>{moment(slot.begin).format("hh:mm A")}</Text> <FiChevronRight style={{ fontSize: "16px", marginInline: "7px" }} /> <Text>{moment(slot.end).format("hh:mm A")}</Text>
                                    </Box>
                                </SlotButton>
                            }) : <Box width={"100%"} height={"100%"} display="flex" flexDirection={"column"} justifyContent={"center"} alignItems="center">

                                <img alt="no availability" style={{ height: "80px" }} src="/images/empty-state-schedule.png" />
                                <Box mt={2}>
                                    <Text whiteSpace={"normal"} as="small" variant={"muted"}>There are no open slots on this day</Text>
                                </Box>
                            </Box>}
                        </DateColContent>
                    </DateCol>
