...
import { Box, Modal, ModalBody, ModalContent, ModalOverlay, Divider, Text, useDisclosure, ModalFooter, Button } from '@chakra-ui/react';
...
width: 40px;
height: 40px;
background: ${props => props.$isDisabled ? theme.colors.red[400] : 'rgba(255, 255, 255, 0.2)'};
border-radius: 12px;
color: #FFF;
display: flex;
justify-content: center;
...
&:hover {
    background: ${props => props.$isDisabled ? theme.colors.red[500] : 'rgba(255, 255, 255, 0.4)'};
}
`
...
const SessionPrefaceDialog = React.forwardRef<SessionPrefaceDialogRef, Props>(({ title, initial }, ref) => {
    ...
    useLayoutEffect(() => {
        if (videoRefSet) {
            if (isOpen)
                startVideo(htmlVideoElementRef.current as HTMLVideoElement);
            else
                stopVideo(htmlVideoElementRef.current as HTMLVideoElement);
        }
    }, [isOpen, videoRefSet])
            ...
            <ModalContent>
                <ModalBody padding={0} paddingBottom={'0 !important'} flexDirection='column'>
                    <Box w={'100%'} mt={5} textAlign='center'>
                        <Text className='sub3' color='text.200'>{title}</Text>
                        <Divider mb={0} orientation='horizontal' />
                    </Box>
                    <Box w={'100%'} p={6} pb={'46px'}>
                        <VideoContainer>
                            <video hidden={videoOff} muted ref={(r) => {
                                htmlVideoElementRef.current = r;
                                setVideoRefSet(!!r);
                            }} autoPlay />
                            <UserInitial>{initial}</UserInitial>
                            <VideoActions>
                                <VideoActionBtn onClick={() => setAudioOff(v => !v)} $isDisabled={audioOff}>{!audioOff ? <FaMicrophone /> : <FaMicrophoneSlash />}</VideoActionBtn>
                                <VideoActionBtn onClick={() => setVideoOff(v => !v)} $isDisabled={videoOff}>{!videoOff ? <FaVideo /> : <FaVideoSlash />}</VideoActionBtn>
                            </VideoActions>
                        </VideoContainer>
                    </Box>
                </ModalBody>
                <ModalFooter>
                    <Button isDisabled={!conferenceUrl} as={'a'} href={conferenceUrl as string}>Join Lesson</Button>
                </ModalFooter>
            </ModalContent>
        </Modal>
    </Root >
