...
import { Box, Modal, ModalBody, ModalCloseButton, Text, ModalContent, ModalHeader, ModalOverlay, useDisclosure, Radio, VStack, Button } from '@chakra-ui/react';
...
const ChoosePaymentMethodDialog = React.forwardRef<ChoosePaymentMethodDialogRef, Props>(({ prefix }, ref) => {
    const { user } = userStore();
    const { isOpen, onOpen, onClose } = useDisclosure();
    const [currentPaymentMethod, setCurrentPaymentMethod] = useState<PaymentMethod | null>(null);
    const promiseResolve = useRef<((value: PaymentMethod | PromiseLike<PaymentMethod>) => void) | null>(null);
        ...
        <Modal isOpen={isOpen} onClose={() => {
            ...
            <ModalContent>
                ...
                <ModalBody flexDirection={'column'}>
                    ...
                    <VStack width={'100%'}>
                        {
                            user?.paymentMethods.map(pm => <CardRoot onClick={() => setCurrentPaymentMethod(pm)} key={pm._id}>
                                {getBrandLogo(pm.brand)}
                                <CardMeta>
                                    <Text>•••• {pm.last4}</Text>
                                    <Text>{pm.expMonth}/{pm.expYear}</Text>
                                </CardMeta>
                                <Radio readOnly isChecked={currentPaymentMethod?._id === pm._id} />
                            </CardRoot>)
                        }
                    </VStack>
                    <Box width={'100%'} mt={5}>
                        <Button onClick={() => {
                            onClose();
                            promiseResolve.current?.(currentPaymentMethod as PaymentMethod);
                        }} type="submit" width={"100%"} variant={"solid"} isDisabled={currentPaymentMethod === null}>Continue</Button>
                    </Box>
                </ModalBody>
            </ModalContent>
        </Modal>
