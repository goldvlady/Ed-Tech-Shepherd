...
const ScheduleBuilderDialog = React.forwardRef<ScheduleBuilderDialogRef, Props>(
    (props, ref) => {
        ...
        const promiseResolve = useRef<
            | ((value: SingleSchedule | PromiseLike<SingleSchedule>) => void)
            | null
        >(null);
        ...
        React.useImperativeHandle(ref, () => {
            return {
                buildSchedule: async (dayOfWeek) => {
                    ...
                    if (dayOfWeek !== null) {
                        setDays([
                            dayOptions.find((v) => v.value === dayOfWeek),
                        ]);
                    }
        ...
        const hoursDiff = moment
            .duration(toTimeDate.diff(fromTimeDate))
            .asHours();

        const canDone =
            !isEmpty(days) && !!fromTime && !!toTime && hoursDiff > 0;
                            ...
                            <Box>
                                        ...
                                        <Box mb={2}>
                                            ...
                                            <Box>
                                                <Text variant={'muted'} mb={0}>
                                                    Select multiple days of the
                                                    week to repeat availability
                                                    across them
                                                </Text>
                                            </Box>
                                        </Box>
                                ...
                                {hoursDiff < 0 && !!fromTime && !!toTime && (
                                    ...
                                    >
                                        <AlertIcon />
                                        <AlertDescription>
                                            The start time should be before the
                                            end time.
                                        </AlertDescription>
                                    </Alert>
                                )}
